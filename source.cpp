...
public:
    Custom(uint8_t level, double vel_x, double vel_y, double vel_w): safe(LeggedType::Go1), udp(level, 8090, "192.168.123.10", 8007) {
        ...
        // policy = torch::jit::load("../go1_rnn_Apr_17.pt");
        policy = torch::jit::load("../go1_rma_p3_Apr_17.pt");
        // policy = torch::jit::load("../go1_apr10.pt");

        // for (int i = 0; i < 100; i++){
            // tensor[0] = torch::rand({1}); 
            //tensor[0] = torch::rand({1,48}); 
    ...
    torch::Tensor rma_obs = torch::rand(420); 
...
{   
	static const int HIST_LEN = 10;
	static std::deque<std::vector<float> > deq_hist;
	if ((int)deq_hist.empty()) {
		for (int i = 0; i < HIST_LEN; i++) {
			deq_hist.push_back(std::vector<float>(42, 0.0));
		}
	}
    start = clock(); 
    motiontime++;
    ...
    if (motiontime <= 100){
	    ...
	    // Put ang vel
			...
			if ((int)deq_hist.size() > HIST_LEN) {
				deq_hist.pop_back();
			}
			for (int i = 0; i < HIST_LEN; i++) {
				for (int j = 0; j < (int)deq_hist[i].size(); j++) {
					rma_obs.index_put_({i * (int)deq_hist[i].size() + j}, deq_hist[i][j]);
				}
			}
            ...
            tensor[0] = tmp_obs.unsqueeze(0); 
            out = policy.forward(tensor).toTensor().squeeze(0); 
